...
const RightPanel: React.FC<RightPanelProps> = ({
  ...
  const downloadableResultsCount = results.filter(r => r.prompt !== 'Original Image' && r.prompt !== 'Image Edited' && !r.prompt.startsWith("Failed:")).length;
  ...
  const getPromptText = (result: ResultItem) => {
      ...
      const translatedPreset = t[result.prompt as keyof typeof t] as string | undefined;
      if (translatedPreset) {
        return translatedPreset;
      }

      if (result.prompt === 'Reanimated') return t.reanimated;
          ...
          results.map((result) => (
            ...
            >
              ...
              />
               ...
               <div className={`absolute bottom-0 left-0 right-0 bg-black/60 p-2 backdrop-blur-sm ${result.prompt.startsWith("Failed:") ? 'bg-red-900/80' : ''}`}>
                <p className="text-xs text-gray-100 truncate" title={result.prompt}>
                    {getPromptText(result)}
                </p>
              </div>
              
              {!result.prompt.startsWith("Failed:") && (
                <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                        onClick={(e) => { e.stopPropagation(); onUpscaleAndDownload(result); }}
                        title={t.upscaleAndDownloadTitle}
                        disabled={result.resultType === 'video'}
                        className="bg-gray-900/50 p-2 rounded-full text-gray-100 hover:bg-yellow-400 hover:text-gray-900 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                    >
      ...
      <div className="mt-4 pt-4 border-t border-gray-700 space-y-3">
        <div className="flex items-center space-x-2">
            <button
              ...
              disabled={!selectedResult || isLoading || selectedResult.prompt.startsWith("Failed:")}
            ...
            <button
              ...
              disabled={!selectedResult || isLoading || selectedResult.prompt.startsWith("Failed:") || selectedResult.resultType === 'video'}
              className="flex-1 flex items-center justify-center bg-gray-700 text-gray-300 font-bold py-2 px-3 text-sm rounded-lg hover:bg-gray-600 disabled:opacity-40 disabled:cursor-not-allowed transition"
            >
        ...
        <button
          ...
          disabled={downloadableResultsCount === 0}
          className="w-full flex items-center justify-center bg-gray-700 text-gray-300 font-bold py-2 px-4 rounded-lg hover:bg-green-600 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-gray-700 transition"
        >
          <DownloadIcon className="h-5 w-5 mr-2" />
          {t.downloadAllResultsBtn} ({downloadableResultsCount})
        </button>
        <button
          onClick={onClearAll}
